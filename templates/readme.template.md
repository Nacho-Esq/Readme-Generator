...
## 1. Resumen [[ resumen | S | Presenta el proyecto a alto nivel para quien llega por primera vez al repositorio: qué es, para qué sirve, qué problema resuelve y en qué estado está. Prioriza una descripción clara y comprensible, sin profundizar en la implementación. Rellena cada campo solo con lo que puedas afirmar con evidencia. ]]
...
## 2. Alcance y limitaciones [[ alcance | S | Delimita las fronteras del proyecto para gestionar expectativas y evitar usos indebidos. Distingue entre lo que el proyecto sí hace, lo que deliberadamente deja fuera y sus limitaciones técnicas conocidas. Rellena estos campos solo con evidencias reales del repositorio. ]]
...
### Errores y fallback [[ ux.fallback_error_handling | M | list | Errores, excepciones, entradas inválidas o casos no cubiertos que puede encontrar el usuario, y cómo responde el proyecto ante cada uno desde el punto de vista del usuario. Un elemento de la lista por cada tipo de error o situación de fallback relevante. ]]

---

## 4. Arquitectura [[ arquitectura | S | Describe la estructura técnica interna del sistema y cómo fluyen los datos, para que una persona desarrolladora entienda sus piezas reales, responsabilidades y dependencias externas. Básate en los componentes reales del repositorio, no en ejemplos genéricos. ]]

### Diagrama lógico [[ architecture.logical_flow | M | raw | Genera un diagrama Mermaid que represente la arquitectura real del proyecto: quién lo invoca, la interfaz de entrada, los componentes internos y los servicios externos, con la dirección principal de los datos (entrada -> procesamiento -> salida). Muestra las partes PRINCIPALES que sepas identificar en el repositorio, agrupándolas con subgraphs si ayuda; incluye toda la arquitectura relevante pero deja fuera lo trivial y no busques un diagrama enorme: prioriza claridad sobre exhaustividad. Usa los nombres reales del código o la documentación, no ejemplos genéricos. SINTAXIS MERMAID OBLIGATORIA: usa identificadores de nodo simples (solo letras y números, sin espacios ni símbolos) y mete TODO el texto visible del nodo entre comillas dobles siempre que contenga paréntesis, arroba, barra, dos puntos, comas, corchetes u otros símbolos; por ejemplo escribe App["index.js / Express app"] y Term["@godaddy/term-protection"], nunca sin comillas, porque sin ellas el flowchart no compila. PROCESOS SÍNCRONOS: si un componente pide datos a otro de forma síncrona (espera la respuesta antes de seguir) y luego usa esos datos, dibuja tanto la conexión de ida como la de vuelta con el dato devuelto; dejar solo la flecha de ida cuando el dato se consume después es un error. Omite la flecha de vuelta únicamente cuando el proceso sea realmente asíncrono (fire-and-forget, sin esperar el resultado), y en ese caso refleja esa asincronía en el diagrama en vez de simplemente omitir el retorno. Debajo, una línea breve de leyenda con el objetivo del diagrama. Si no puedes generarlo, deja una alternativa textual de máximo 3 líneas. ]]
...
## 7. Desarrollo local [[ desarrollo-local | S | Explica cómo ejecutar el proyecto en una máquina local partiendo de cero: requisitos, configuración, datos necesarios, comandos y forma de comprobar que funciona. Usa pasos concretos y reproducibles sacados de los scripts, manifests y documentación del repositorio. ]]

### Requisitos [[ local_development.requirements | M | raw | Enumera los requisitos previos que hay que tener instalados para desarrollar el proyecto en local (runtimes como Python/Node/Java, gestores de paquetes, Docker, bases de datos locales, CLIs u otras herramientas del sistema), con su versión exacta cuando conste en el repositorio. Para cada requisito no te limites a nombrarlo: explica también cómo instalarlo. Formato por requisito: una línea en negrita con el nombre y la versión (p. ej. "**Python 3.11**"), seguida de un bloque fenced de código con el/los comando(s) de instalación reales para esa versión. Ese bloque debe contener SOLO comandos que instalan o configuran el requisito; nunca metas ahí una comprobación tipo "--version" como si fuera parte de la instalación, porque no lo es. Si el repositorio evidencia un gestor de versiones (pyenv, nvm, asdf, volta), usa sus comandos; si no, usa el método de instalación oficial más común. Cuando la instalación difiera claramente según el sistema operativo, prioriza el del SO que use el proyecto (según Dockerfile o scripts) y menciona brevemente las alternativas, sin construir una tabla exhaustiva por SO. Después del bloque de instalación, si ayuda, añade una nota breve EXPLÍCITAMENTE etiquetada como comprobación posterior (p. ej. "Comprobación tras instalar: `node --version` debe mostrar la versión instalada"): dicha nota verifica que la instalación salió bien, no es un paso para instalarlo, así que no la presentes ni la redactes como si lo fuera. Incluye solo requisitos con evidencia real; no inventes versiones ni herramientas. No cubras aquí la instalación de las dependencias del propio proyecto (p. ej. npm install o pip install -r), que va en "Ejecutar". ]]
...
## 8. Seguridad y privacidad [[ seguridad | S | Documenta cómo trata el proyecto los datos: qué información maneja, dónde se almacena, quién puede acceder a ella y cómo se protegen las credenciales y los datos sensibles. Afirmar algo incorrecto aquí es especialmente delicado, así que rellena solo con evidencias claras y, ante la duda, deja el campo vacío. ]]
...
## 9. Despliegue [[ despliegue | S | Describe cómo se lleva el proyecto a entornos reales de ejecución: qué hay que provisionar antes, qué entornos existen y cómo se despliega. Básate en los pipelines, contenedores, IaC y scripts de despliegue del repositorio. ]]
...
## 10. Documentación y enlaces [[ documentacion | S | Reúne, a modo de índice, la documentación y los enlaces útiles del proyecto: manuales, repositorios, pipelines, dashboards y demás recursos. Incluye solo referencias que existan de verdad en el repositorio. ]]
...
## 11. Roadmap / mejoras [[ mejoras | S | Evolución prevista del proyecto. ]]

[[ roadmap | H | list | Mejoras futuras o elementos de roadmap.]]

---

## 12. Contactos y ownership [[ contactos | S | Personas y equipos responsables del proyecto. ]]
